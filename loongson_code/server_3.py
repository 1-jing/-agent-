...
# ================= 局域网 UDP 自动发现 (核心新增) =================
def udp_broadcaster():
    """后台独立线程：不断向局域网广播自己的 IP 地址"""
    udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udp_socket.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
    print("📡 UDP 广播服务已启动，等待龙芯派自动连接...")
    while True:
        try:
            # 向局域网内所有设备发送暗号
            udp_socket.sendto(b"AI_SERVER_HERE", ("255.255.255.255", 9999))
        except Exception:
            pass
        time.sleep(2)
...
async def get_ai_reply(question: str, sensor_data: Optional[dict] = None) -> dict:
    ...
    if sensor_data and sensor_data.get("T") is not None:
        ...
        sensor_context = f"【实时传感器数据】温度：{temp}°C（状态：{temp_status}），湿度：{humi}%（状态：{humi_status}）。"
    else:
